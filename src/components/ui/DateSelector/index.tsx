...
const DateSelector = forwardRef<ClearableHandle, DateSelectorProps>(
    ({ initialValue, onDateChange }, ref) => {
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        const [date, setDate] = useState<Date | null>(() => {
            if (!initialValue) return null;
            return parseDateFromInput(initialValue, DATE_FORMAT);
        });
        const [dateStr, setDateStr] = useState<string>(initialValue ?? "");
        const [yearMonth, setYearMonth] = useState<YearMonthState>(() => {
            const parsed = initialValue ? parseDateFromInput(initialValue, DATE_FORMAT) : null;
            return {
                year: parsed?.getFullYear() ?? today.getFullYear(),
                month: parsed ? parsed.getMonth() + 1 : today.getMonth() + 1,
            };
        });
        ...
        // Emit whenever internal date state changes (set via input blur or calendar pick)
        useEffect(() => {
            onDateChange(date);
        }, [date]);
        ...
        function completeDateInput(e: React.FocusEvent<HTMLInputElement>) {
            ...
            const dayStr = parts[segmentNames.indexOf("day")!] ?? "";
            const monthStr = parts[segmentNames.indexOf("month")!] ?? "";
            const yearStr = parts[segmentNames.indexOf("year")!] ?? "";
            ...
            return segments.map(seg => {
                switch (seg.name) {
                    case "day":   return day.toString().padStart(seg.end - seg.start, "0");
                    case "month": return month.toString().padStart(seg.end - seg.start, "0");
        ...
        function handleBackspace(e: React.KeyboardEvent<HTMLInputElement>) {
            ...
            if (slashPositions.includes(pos)) {
                e.preventDefault();
                setDateStr(prev => (prev.endsWith("/") ? prev.slice(0, -1) : prev));
            }
        }
        ...
        function handleTab(e: React.KeyboardEvent<HTMLInputElement>) {
            ...
            if (isEmpty) {
                switch (name) {
                    case "day":   tabValue = today.getDate().toString().padStart(2, "0"); break;
                    case "month": tabValue = (today.getMonth() + 1).toString().padStart(2, "0"); break;
                    case "year":  tabValue = today.getFullYear().toString(); break;
                }
                e.preventDefault();
            } else {
                if (segment.end === 10) {
                ...
                if (name === "year") {
                    const yearValue = Number.parseInt(tabValue);
            ...
            setDateStr(normalizeDateInput(`${startValue}${tabValue}`));
        }

        return (
            <div ref={setReference} className={styles.date_selector}>
                <MaskedInput
                    ref={inputRef}
                    ...
                    onChange={(value) => setDateStr(normalizeDateInput(value))}
                />
